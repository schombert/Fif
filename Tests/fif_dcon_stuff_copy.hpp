...
" :struct vpool-thingy-pooled_v ptr(i32) content ;  "
" :s pooled_v thingy_id s: >index 4 * " + std::to_string(offsetof(dcon::data_container, thingy) + offsetof(dcon::internal::thingy_class, m_pooled_v)+ offsetof(dcon::internal::thingy_class::dtype_pooled_v, values)) + " + data-container @ buf-add ptr-cast ptr(i32) make vpool-thingy-pooled_v .content! ; "
" :s size vpool-thingy-pooled_v s: .content @ dup 1 + >i32 0 = if drop 0 else 8 * 4 + vector-storage @ buf-add ptr-cast ptr(u16) @ >i32 then ;  "
" :s index vpool-thingy-pooled_v i32 s: let idx .content @ 8 * 8 + " + std::to_string(sizeof(int16_t)) + " idx * + vector-storage @ buf-add ptr-cast ptr(i16) ;  "
...
; result += "" 
" :s _index person_id s: >index " + std::to_string(sizeof(dcon::person_id)) + " * " + std::to_string(offsetof(dcon::data_container, person) + offsetof(dcon::internal::person_class, m__index)+ offsetof(dcon::internal::person_class::dtype__index, values)) + " + data-container @ buf-add ptr-cast ptr(person_id) ; "
" :s live? person_id s: dup _index @ = ; "
...
" :struct vpool-car_ownership-owner ptr(i32) content ;  "
" :s car_ownership-owner person_id s: >index 4 * " + std::to_string(offsetof(dcon::data_container, car_ownership) + offsetof(dcon::internal::car_ownership_class, m_array_owner)+ offsetof(dcon::internal::car_ownership_class::dtype_array_owner, values)) + " + data-container @ buf-add ptr-cast ptr(i32) make vpool-car_ownership-owner .content! ; "
" :s size vpool-car_ownership-owner s: .content @ dup 1 + >i32 0 = if drop 0 else 8 * 4 + vector-storage @ buf-add ptr-cast ptr(u16) @ >i32 then ;  "
" :s index vpool-car_ownership-owner i32 s: let idx .content @ 8 * 8 + " + std::to_string(sizeof(dcon::car_ownership_id)) + " idx * + vector-storage @ buf-add ptr-cast ptr(car_ownership_id) ;  "
